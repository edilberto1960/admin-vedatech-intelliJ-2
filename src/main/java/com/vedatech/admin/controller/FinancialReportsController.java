...
import com.vedatech.admin.model.ResultData3;
...
@RestController
@RequestMapping(value = "/financial_reports")
public class FinancialReportsController {
  ...
  /*  @RequestMapping(value = "/getAllTransactions/{id}", method = RequestMethod.GET)
   ...
   // @RequestMapping(value = "/incomeStatements/", method = RequestMethod.GET)
    ...
    @RequestMapping(value = "/incomeStatements/", method = RequestMethod.POST)
    public ResponseEntity<List<Object[]>> createUser(@RequestBody GetTransaction getTransaction, UriComponentsBuilder ucBuilder) {
      ...
      //  List<Transactions> transactionsList = transactionService.findTransactionsbyDate(getTransaction.getBank().getId(), getTransaction.getInitialDate(), getTransaction.getFinalDate());
        ...
        List<Object[]> objectsDeposit = transactionService.getSumDepositAmountByDepartmentByType(getTransaction.getInitialDate(), getTransaction.getFinalDate());
    ...
    @RequestMapping(value = "/incomeDeposit/", method = RequestMethod.POST)
    public ResponseEntity<List<Object[]>> getSalesReport(@RequestBody GetTransaction getTransaction, UriComponentsBuilder ucBuilder) {
        System.out.println("GET REPORTS_Deposit: " + getTransaction.toString());
        List<Object[]> objects = transactionService.getSumDepositAmountByDepartmentByType(getTransaction.getInitialDate(), getTransaction.getFinalDate());
